...
class CheckJobTitlesTestCase(FacultyBiosControllerBaseTestCase):
    #######################
    ### Utility methods ###
    #######################

    def __init__(self, methodName):
        super(CheckJobTitlesTestCase, self).__init__(methodName)

    #######################
    ### Testing methods ###
    #######################

    def test_check_job_titles_valid(self):
        test_form = {
            'website': u'None.',
            'image': u'',
        ...
        response = self.controller.check_job_titles(test_form)
        self.assertTrue(isinstance(response, tuple))
        self.assertEqual(len(response), 3)
        self.assertTrue(isinstance(response[0], bool))
